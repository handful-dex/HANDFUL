...
from envs.config.xarm7_allegro_config import (PICK_CUBE_CONFIGS, PICK_REWARD_CONFIGS, TWO_PICK_CONFIGS, TWO_PICK_REWARD_CONFIGS,
...
@register_env("xArm7-v1-two-pick-unified", max_episode_steps=MAX_EPISODE_STEPS)
class XArm7TableTopTwoPickUnified(BaseEnv):
    """
    Unified environment combining two sequential picking tasks:
    Phase 1 (0-74 steps): Pick and place first cube to goal
    Phase 2 (75-174 steps): Pick second object and place at push goal
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.two_pick_reward = TWO_PICK_REWARD_CONFIGS["sac"] if self.reward_type is None else TWO_PICK_REWARD_CONFIGS[self.reward_type]
        ...
        if self.robot_uids in TWO_PICK_CONFIGS:
            two_pick_cfg = TWO_PICK_CONFIGS[self.robot_uids]
        else:
            two_pick_cfg = TWO_PICK_CONFIGS["panda"]
        ...
        self.push_block_variation_dimensions = two_pick_cfg["push_block_variation_dimensions"]
        self.push_block_spawn_center = two_pick_cfg["push_block_spawn_center"]
        self.push_block_rot = two_pick_cfg["push_block_rot"]
        self.max_push_block_goal_height = two_pick_cfg["max_goal_height"]

        self.human_cam_eye_pos = two_pick_cfg["human_cam_eye_pos"]
        self.human_cam_target_pos = two_pick_cfg["human_cam_target_pos"]
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            random_xy = self._batched_episode_rng.uniform(
                low=-3*self.cube_spawn_half_size,
                high=3*self.cube_spawn_half_size,
                size=(2,)
            )
            ...
            push_goal_xyz[:, 2] = self.max_push_block_goal_height
            ...
            if self.finger_selection == "random":
                ...
                shuffled_fingers = torch.tensor([3,1,0,2], device=self.device).unsqueeze(0).expand(b, -1)
                self.palm_use = False
                self.num_active_fingers = 2
    ...
    def _compute_phase2_reward(self, obs, action, info):
        ...
        # 7. Final reward
        reward = (
            self.two_pick_reward["grasp_reward"] * grasp_reward +
            self.two_pick_reward["reach_weight"] * reaching_reward +
            self.two_pick_reward["finger_dist_weight"] * combined_push_dist_reward +
            self.two_pick_reward["push_block_grasp_weight"] * push_block_grasp_reward +
            self.two_pick_reward["inactive_finger_penalty_weight"] * inactive_finger_penalty +
            self.two_pick_reward["height_weight"] * height_reward + 
            self.two_pick_reward["place_weight"] * place_reward + 
            self.two_pick_reward["static_penalty_weight"] * static_penalty +
            self.two_pick_reward["collision_penalty_weight"] * collision_penalty
        )
    ...
    def compute_normalized_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        sum_phase2 = 5.0 + sum(self.two_pick_reward[k] for k in 
                              ["grasp_reward", "reach_weight", "finger_dist_weight", 
                               "push_block_grasp_weight", "height_weight", "place_weight"])
