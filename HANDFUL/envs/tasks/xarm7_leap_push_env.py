...
from envs.config.xarm7_allegro_config import (PICK_PUSH_CONFIGS, PICK_PUSH_REWARD_CONFIGS,
...
DEFAULT_ACTIVE_FINGER_INDICES = [[0,3]]
DEFAULT_INACTIVE_FINGER_INDICES = [[1,2]]


@register_env("xArm7-v1-push", max_episode_steps=100)
class XArm7TableTopPush(BaseEnv):
    ...
    def __init__(self, *args, state_file_path: str = "",  **kwargs):
        ...
        self.reward_type = PICK_PUSH_REWARD_CONFIGS["sac"] if self.reward_type is None else PICK_PUSH_REWARD_CONFIGS[self.reward_type]
        self.difficulty = 3 if self.difficulty is None else self.difficulty

        if self.robot_uids in PICK_PUSH_CONFIGS:
            cfg = PICK_PUSH_CONFIGS[self.robot_uids]
        else:
            cfg = PICK_PUSH_CONFIGS["panda"]
    ...
    def _default_sim_config(self):
        sim_config = SimConfig(
            gpu_memory_config=GPUMemoryConfig(
                max_rigid_contact_count=self.num_envs * max(2048, self.num_envs) * 64,
                max_rigid_patch_count=self.num_envs * max(2048, self.num_envs) * 32,
    ...
    def evaluate(self):
        ...
        is_obj_placed = (
            torch.linalg.norm(self.push_goal_site.pose.p - self.push_block.pose.p, axis=1) <= self.goal_thresh
        )
        is_obj_rotated = (
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        reward = (
            ...
            self.reward_type["push_weight"] * push_grasp_reward +
            self.reward_type["place_weight"] * place_reward +
            self.reward_type["rot_weight"] * rot_reward +
    ...
    def compute_normalized_dense_reward(
        ...
        sum_rewards = 5.0
        for key in ["reach_weight", "grasp_weight", "rot_weight", "place_weight"]:
