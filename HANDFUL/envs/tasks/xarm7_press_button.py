...
from envs.config.xarm7_allegro_config import (PICK_PRESS_CONFIGS, PICK_PRESS_REWARD_CONFIGS, 
...
DEFAULT_ACTIVE_FINGER_INDICES = [[0]]
DEFAULT_INACTIVE_FINGER_INDICES = [[1,2,3]]  # Empty - all fingers active for pressing
...
@register_env("xArm7-v1-press-button", max_episode_steps=100)
class XArm7TableTopPress(BaseEnv):
    ...
    def __init__(self, *args, state_file_path: str = "",  **kwargs):
        ...
        self.reward_type = PICK_PRESS_REWARD_CONFIGS["sac"] if self.reward_type is None else PICK_PRESS_REWARD_CONFIGS[self.reward_type]
        self.difficulty = 3 if self.difficulty is None else self.difficulty

        if self.robot_uids in PICK_PRESS_CONFIGS:
            cfg = PICK_PRESS_CONFIGS[self.robot_uids]
        else:
            cfg = PICK_PRESS_CONFIGS["panda"]
        self.cube_half_size = 0.025

        self.cube_variation_dimensions = cfg["cube_variation_dimensions"]
        self.push_block_spawn_half_size = cfg["push_block_spawn_half_size"]
        self.push_block_spawn_rot_noise = cfg["push_block_spawn_rot_noise"]
        ...
        # obstacle
        self.box_with_hole_radii_and_depth = cfg["box_with_hole_radii_and_depth"]
    ...
    def evaluate(self):
        ...
        is_button_pressed = (min_finger_dist < self.goal_thresh)
