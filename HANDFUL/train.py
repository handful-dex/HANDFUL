...
class Args:
    ...
    # picking specific args - usually filled in by unified training file
    finger_selection: Optional[str] = None
    """finger selection mode: 'random', 'fixed', or list"""
    palm_use: bool = False
    """whether to use palm contact in grasping"""
    num_active_fingers: int = 2

    # second task specific args - usually filled in by unified training file
    state_file_path: Optional[str] = None
    """path to intermediate states file for second tasks (if applicable)"""
    difficulty: Optional[int] = None
    """difficulty of second task (if using curriculum)"""
...
if __name__ == "__main__":
    ...
    if args.finger_selection is not None:
        if ',' in args.finger_selection:
            env_kwargs["finger_selection"] = [int(x) for x in args.finger_selection.split(',')]
        else:
            env_kwargs["finger_selection"] = args.finger_selection
    ...
    if args.capture_video or args.save_trajectory:
        ...
        if args.save_train_video_freq is not None:
            ...
            envs = RecordEpisode(envs, output_dir=f"runs/{run_name}/train_videos", save_trajectory=False, save_video_trigger=save_video_trigger, max_steps_per_video=args.num_steps, video_fps=30, info_on_video=True)
    ...
    while global_step < args.total_timesteps:
        ...
        for local_step in range(args.steps_per_env):
            ...
            if not learning_has_started:
                if args.checkpoint is not None:
                    # use loaded policy with noise for diverse but meaningful prefill
                    with torch.no_grad():
            ...
            logger.add_scalar(f"train/reward", rewards.float().mean(), global_step)
